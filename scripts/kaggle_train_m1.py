"""M1 training kernel, run on Kaggle GPU. Templated: {{SEED}} is substituted by
scripts/run_m1.py before each `kaggle kernels push` - this file is never pushed
as-is, and is not meant to be run locally (no local GPU training of M1, per
project policy).

Identical pipeline to scripts/kaggle_train_m0.py. The only intended difference
between M0 and M1 is the config the model factory reads
(src.config.EXPERIMENT_CONFIGS["M1"]["bidirectional"] = True) - this script does
not branch on experiment identity anywhere beyond that.
"""
...
INPUT_DIR = Path("/kaggle/input/datasets/udbhav748/cfpb-m1-frozen-inputs")
...
def main() -> None:
    ...
    # 3. Frozen tokenizer (never refit) and sequences at M1's locked max_len=128 -
    # same tokenizer, same vocabulary, same max_len as M0.
    ...
    # 4. Build M1 - random embeddings, no matrix passed. The only architecture
    # difference from M0 is config.bidirectional=True, which build_recurrent_model
    # wraps the LSTM in Bidirectional() for - not a branch in this script.
    set_seed(SEED)
    ...
    # 6. Callbacks - for M1 this is exactly [ModelCheckpoint(val_macro_f1)],
    # enforced by the factory itself (no scheduler/early-stopping fields are set
    # in M1's config, so build_callbacks adds neither).
    ...
    run_record = {
        "experiment": EXPERIMENT, "model": "Bidirectional LSTM",
