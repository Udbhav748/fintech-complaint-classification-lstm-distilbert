"""Builds and uploads the Kaggle Dataset the M3 training kernel reads from.

Same pattern as scripts/kaggle_package_m2.py, with one addition: M3 uses GloVe
embeddings, so the frozen matrix + metadata built in Task 3
(artifacts/embeddings/) is packaged too - M0-M2 never needed it since they use
random embeddings. The matrix is shipped as-is, never rebuilt on Kaggle.
...
the frozen one.
    ...
    python scripts/kaggle_package_m3.py
"""
...
def verify_local_inputs() -> None:
    ...
    a bad local file is caught before spending an upload on it. Also confirms
    the frozen GloVe matrix shape matches (vocab_size, embedding_dim) - it is
    packaged as-is, never rebuilt, so a shape mismatch here is a real bug."""
    df_raw = load_raw()
...
def upload() -> str:
    ...
    if dataset_exists():
        result = subprocess.run(
            ["kaggle", "datasets", "version", "-p", str(STAGING_DIR), "-m", "M3 training run", "-r", "zip"],
