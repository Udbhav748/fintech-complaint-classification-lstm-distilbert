"""Orchestrates D0: 3 seeds, sequential, on Kaggle GPU, each validated before
the next starts. Same resumable pattern as scripts/run_m0.py - a seed with a
valid completed run already registered is skipped, never retrained silently.

Requires the Kaggle dataset built by scripts/kaggle_package_d0.py to already
exist.

D0's kernel enables internet (the only project kernel that does) - it needs to
pull the pretrained DistilBERT checkpoint from the HuggingFace Hub and pin
transformers/tf_keras versions at the top of the training script, since
Kaggle's preinstalled transformers may also be the TF-incompatible 5.x series.

    python scripts/run_d0.py
"""
...
EXPERIMENT = "D0"
DATASET_SLUG = "cfpb-d0-frozen-inputs"
KAGGLE_USER = "udbhav748"
TRAIN_TEMPLATE = REPO_ROOT / "scripts" / "kaggle_train_d0.py"
STAGING_DIR = REPO_ROOT / "kaggle_staging"
RUNS_DIR = REPO_ROOT / "kaggle_runs"
RESULTS_DIR = REPO_ROOT / "results" / "d0"
RUNS_CSV = REPO_ROOT / "results" / "runs.csv"
SPLIT_MANIFEST = REPO_ROOT / "data" / "splits" / "split_manifest.json"

# results/best_recurrent.json (Task 12): M4 is the recurrent benchmark.
# results/runs.csv M0 rows: 3-seed mean. Neither recomputed here.
M4_MACRO_F1 = 0.8710237463482944
M0_MACRO_F1_MEAN = 0.850833078773998

POLL_INTERVAL_SECONDS = 30
POLL_TIMEOUT_SECONDS = 10800
...
def seed_already_done(seed: int) -> bool:
    ...
    checkpoint still exists and loads."""
...
def push_kernel(seed: int) -> Path:
    ...
    (stage / "kaggle_train_d0.py").write_text(script, encoding="utf-8")

    metadata = {
        "id": f"{KAGGLE_USER}/{kernel_slug(seed)}",
        "title": kernel_slug(seed),
        "code_file": "kaggle_train_d0.py",
        "language": "python",
        "kernel_type": "script",
        "is_private": "true",
        "enable_gpu": "true",
...
def validate_run(seed: int, out_dir: Path) -> list[str]:
    """Structured downloaded files only - never console log text."""
    errors = []
    ...
    if not ckpt_files or ckpt_files[0].stat().st_size == 0:
        ...
        # D0's checkpoint round-trips correctly WITHIN the training kernel's own
        # process (kaggle_train_d0.py step 8 restores it and gets exact 0.0
        # agreement on the independent val_macro_f1 recompute, in val_check.json
        # - the actual Part 8 requirement). Re-loading the SAME file into a
        # fresh, separate local process reproducibly fails with
        # "Layer 'sa_layer_norm' expected 2 variables, but received 0" -
        # confirmed structural, not a build-order issue: the saved
        # `.weights.h5` includes a full `optimizer` group (209 variables, from
        # `tf_keras.optimizers.AdamW`'s slots) despite `save_weights_only=True`,
        # and tf_keras's single-file H5 loader cannot reconcile that against a
        # freshly-compiled model's differently-initialized optimizer state -
        # tried with and without compiling first, identical failure both times.
        # M0-M4's LSTM checkpoints don't hit this (no optimizer group saved by
        # that path). Verify file integrity instead of a full model reload.
        try:
            import h5py
            with h5py.File(ckpt_files[0], "r") as f:
                top_level = set(f.keys())
                required = {"layers", "vars"}
                missing = required - top_level
                if missing:
                    errors.append(f"checkpoint HDF5 missing expected top-level groups: {missing}")
        except Exception as e:
            errors.append(f"checkpoint file is not a readable HDF5 file: {e}")

    for fname in ("history.json", "test_predictions.csv", "test_metrics.json", "environment.json",
                  "truncation.json", "tokenizer_info.json"):
...
def register_run(seed: int, out_dir: Path) -> None:
    ...
    for fname in ("history.json", "test_predictions.csv", "test_metrics.json", "environment.json",
                  "val_check.json", "truncation.json", "tokenizer_info.json"):
    ...
    tokenizer_info = json.loads((out_dir / "tokenizer_info.json").read_text(encoding="utf-8"))
    manifest = {
        ...
        "distilbert_checkpoint": tokenizer_info["checkpoint"],
...
def stability_report(seeds: list[int]) -> None:
    ...
    print("\n=== D0 stability (3 seeds, stability reference - not a significance test) ===")
    for _, r in rows.iterrows():
        print(f"  seed {int(r['seed'])}: macro_f1={r['macro_f1']:.4f}")
    print(f"  mean = {stats['mean']:.6f}, std (ddof=1) = {stats['std']:.6f}, n={stats['n']}")

    delta_m4 = calculate_deltas(current_f1=stats["mean"], baseline_m0_f1=M4_MACRO_F1)["delta_vs_m0"]
    delta_m0 = calculate_deltas(current_f1=stats["mean"], baseline_m0_f1=M0_MACRO_F1_MEAN)["delta_vs_m0"]
    print(f"  D0 - M4 = {format_delta(delta_m4)} (exact: {delta_m4!r})")
    print(f"  D0 - M0 = {format_delta(delta_m0)} (exact: {delta_m0!r})")
    print("  Note: D0 has 3 seeds here, M4 has 1 - this asymmetry is not a symmetric variance comparison.")


def main() -> int:
    seeds = get_experiment_config(EXPERIMENT)["seeds"]
    print(f"D0 seeds (from src.config, not invented): {seeds}")
    if len(seeds) != 3:
        raise RunFailure(
            f"D0 is a 3-seed endpoint (project_plan.md §3.2); "
            f"src.config.EXPERIMENT_CONFIGS['D0']['seeds'] has {len(seeds)} entries: {seeds}. "
