"""Orchestrates M3: single seed, on Kaggle GPU, validated before registration.
Same pattern as scripts/run_m2.py. Resumable - a seed with a valid completed run
already registered is skipped, never retrained silently.

Requires the Kaggle dataset built by scripts/kaggle_package_m3.py to already
exist (run that first, once, or after any frozen input changes - it never should
change mid-M3).

M3 is a single-seed intermediate rung (project_plan.md §3.2) - the seed list
comes from src.config.EXPERIMENT_CONFIGS["M3"]["seeds"], never invented here.
...
EXPERIMENT = "M3"
DATASET_SLUG = "cfpb-m3-frozen-inputs"
KAGGLE_USER = "udbhav748"
TRAIN_TEMPLATE = REPO_ROOT / "scripts" / "kaggle_train_m3.py"
STAGING_DIR = REPO_ROOT / "kaggle_staging"
RUNS_DIR = REPO_ROOT / "kaggle_runs"
RESULTS_DIR = REPO_ROOT / "results" / "m3"
RUNS_CSV = REPO_ROOT / "results" / "runs.csv"
SPLIT_MANIFEST = REPO_ROOT / "data" / "splits" / "split_manifest.json"

# project_plan.md §14.7: M0 mean Macro-F1 = 0.8508. §14.10: M2 = 0.8517974977710514.
# Neither recomputed here.
M0_MACRO_F1 = 0.8508
M2_MACRO_F1 = 0.8517974977710514

POLL_INTERVAL_SECONDS = 30
POLL_TIMEOUT_SECONDS = 10800  # M2 took ~113 min under recurrent_dropout; give headroom.
...
def push_kernel(seed: int) -> Path:
    ...
    (stage / "kaggle_train_m3.py").write_text(script, encoding="utf-8")

    metadata = {
        "id": f"{KAGGLE_USER}/{kernel_slug(seed)}",
        "title": kernel_slug(seed),
        "code_file": "kaggle_train_m3.py",
        "language": "python",
        "kernel_type": "script",
        "is_private": "true",
        "enable_gpu": "true",
...
def validate_run(seed: int, out_dir: Path) -> list[str]:
    ...
    if not ckpt_files or ckpt_files[0].stat().st_size == 0:
        ...
        try:
            ...
            errors.append(f"checkpoint failed to load into a freshly-built M3 model: {e}")
...
def register_run(seed: int, out_dir: Path) -> None:
    ...
    # checkpoint actually lives now, relative to REPO_ROOT (matches M0-M2).
    ...
    delta_m2 = calculate_deltas(current_f1=record["macro_f1"], baseline_m0_f1=M2_MACRO_F1)
    delta_m0 = calculate_deltas(current_f1=record["macro_f1"], baseline_m0_f1=M0_MACRO_F1)
    print(f"M3 - M2 = {format_delta(delta_m2['delta_vs_m0'])} (exact: {delta_m2['delta_vs_m0']!r})")
    print(f"M3 - M0 = {format_delta(delta_m0['delta_vs_m0'])} (exact: {delta_m0['delta_vs_m0']!r})")
...
def main() -> int:
    ...
    print(f"M3 seeds (from src.config, not invented): {seeds}")
    if len(seeds) != 1:
        raise RunFailure(
            f"M3 is a single-seed rung (project_plan.md §3.2/§4); "
            f"src.config.EXPERIMENT_CONFIGS['M3']['seeds'] has {len(seeds)} entries: {seeds}. "
