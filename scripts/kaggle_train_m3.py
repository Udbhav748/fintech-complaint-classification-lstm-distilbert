"""M3 training kernel, run on Kaggle GPU. Templated: {{SEED}} is substituted by
scripts/run_m3.py before each `kaggle kernels push` - this file is never pushed
as-is, and is not meant to be run locally (no local GPU training of M3, per
project policy).

Identical pipeline to scripts/kaggle_train_m2.py. The only intended difference
between M2 and M3 is the embedding source: the frozen GloVe matrix built in
Task 3 is loaded as-is and passed to build_recurrent_model - never rebuilt from
the raw GloVe text file here, so this run uses exactly the matrix already
verified locally (src.embeddings.build_embedding_matrix is not called by this
script at all).

recurrent_dropout stays at 0.2 (unchanged from M2), so this run is expected to
take roughly as long as M2's (~113 minutes), not M1's few minutes.
"""
...
INPUT_DIR = Path("/kaggle/input/datasets/udbhav748/cfpb-m3-frozen-inputs")
...
def main() -> None:
    ...
    # 3. Frozen tokenizer (never refit) and sequences at M3's locked max_len=128.
    ...
    # 3b. Frozen GloVe matrix (Task 3), loaded as-is - never rebuilt here.
    ...
    # 4. Build M3 - GloVe embeddings from the loaded matrix. The only
    # architecture difference from M2 is config.use_glove=True /
    # embedding_matrix passed in - not a branch in this script.
    set_seed(SEED)
    ...
    # 6. Callbacks - for M3 this is exactly [ModelCheckpoint(val_macro_f1)],
    # enforced by the factory itself (no scheduler/early-stopping fields are set
    # in M3's config, so build_callbacks adds neither).
    ...
    # recurrent_dropout=0.2 (unchanged from M2) keeps the cuDNN fast path
    # disabled, so this is expected to run about as long as M2 did.
    history = model.fit(
        X_train, y_train,
    ...
    run_record = {
        "experiment": EXPERIMENT, "model": "BiLSTM + Pretrained GloVe-100d",
