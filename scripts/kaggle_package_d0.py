"""Builds and uploads the Kaggle Dataset the D0 training kernel reads from.

Same verify-then-stage-then-upload pattern as scripts/kaggle_package_m*.py,
but a smaller package: D0 uses the pretrained DistilBERT tokenizer (downloaded
from the HuggingFace Hub at training time, on Kaggle, with internet enabled -
see scripts/kaggle_train_d0.py), not the frozen Keras tokenizer, and random/
GloVe embeddings are irrelevant to a Transformer. Only the frozen dataset,
split, and config are needed here.

    python scripts/kaggle_package_d0.py
"""
...
def upload() -> str:
    ...
    if dataset_exists():
        result = subprocess.run(
            ["kaggle", "datasets", "version", "-p", str(STAGING_DIR), "-m", "D0 training run", "-r", "zip"],
