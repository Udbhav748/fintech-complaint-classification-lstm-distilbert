"""Builds and uploads the Kaggle Dataset the M2 training kernel reads from.

Same pattern as scripts/kaggle_package_m1.py. M2 also uses random embeddings
(src.config.EXPERIMENT_CONFIGS["M2"]["use_glove"] is False) - dropout and
recurrent_dropout are training-time regularization, not an input artifact, so
the packaged inputs are identical in kind to M0/M1's.
...
the frozen one.
    ...
    python scripts/kaggle_package_m2.py
"""
...
def upload() -> str:
    ...
    if dataset_exists():
        result = subprocess.run(
            ["kaggle", "datasets", "version", "-p", str(STAGING_DIR), "-m", "M2 training run", "-r", "zip"],
