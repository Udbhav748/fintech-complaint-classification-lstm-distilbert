"""Builds and uploads the Kaggle Dataset the M4 training kernel reads from.

Same pattern as scripts/kaggle_package_m3.py, including the frozen GloVe
matrix - M4 uses GloVe like M3 (only max_len and the callback set differ,
neither of which changes what needs to be packaged).
...
the frozen one.
    ...
    python scripts/kaggle_package_m4.py
"""
...
def upload() -> str:
    ...
    if dataset_exists():
        result = subprocess.run(
            ["kaggle", "datasets", "version", "-p", str(STAGING_DIR), "-m", "M4 training run", "-r", "zip"],
