"""Orchestrates M1: single seed, on Kaggle GPU, validated before registration.
Same pattern as scripts/run_m0.py. Resumable - a seed with a valid completed run
already registered is skipped, never retrained silently.

Requires the Kaggle dataset built by scripts/kaggle_package_m1.py to already
exist (run that first, once, or after any frozen input changes - it never should
change mid-M1).

M1 is a single-seed intermediate rung (project_plan.md §3.2) - the seed list
comes from src.config.EXPERIMENT_CONFIGS["M1"]["seeds"], never invented here.
...
TRAIN_TEMPLATE = REPO_ROOT / "scripts" / "kaggle_train_m1.py"
STAGING_DIR = REPO_ROOT / "kaggle_staging"
RUNS_DIR = REPO_ROOT / "kaggle_runs"
RESULTS_DIR = REPO_ROOT / "results" / "m1"
...
def push_kernel(seed: int) -> Path:
    ...
    (stage / "kaggle_train_m1.py").write_text(script, encoding="utf-8")

    metadata = {
        "id": f"{KAGGLE_USER}/{kernel_slug(seed)}",
        "title": kernel_slug(seed),
        "code_file": "kaggle_train_m1.py",
        "language": "python",
        "kernel_type": "script",
        "is_private": "true",
        "enable_gpu": "true",
...
def validate_run(seed: int, out_dir: Path) -> list[str]:
    ...
    if not ckpt_files or ckpt_files[0].stat().st_size == 0:
        ...
        try:
            ...
            errors.append(f"checkpoint failed to load into a freshly-built M1 model: {e}")
...
def register_run(seed: int, out_dir: Path) -> None:
    ...
    delta = calculate_deltas(current_f1=record["macro_f1"], baseline_m0_f1=M0_MACRO_F1)
    print(f"M1 - M0 = {format_delta(delta['delta_vs_m0'])} (exact: {delta['delta_vs_m0']!r})")
...
def main() -> int:
    ...
    print(f"M1 seeds (from src.config, not invented): {seeds}")
    if len(seeds) != 1:
        raise RunFailure(
            f"M1 is a single-seed rung (project_plan.md §3.2/§4); "
            f"src.config.EXPERIMENT_CONFIGS['M1']['seeds'] has {len(seeds)} entries: {seeds}. "
