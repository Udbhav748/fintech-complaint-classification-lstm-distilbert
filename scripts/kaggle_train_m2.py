"""M2 training kernel, run on Kaggle GPU. Templated: {{SEED}} is substituted by
scripts/run_m2.py before each `kaggle kernels push` - this file is never pushed
as-is, and is not meant to be run locally (no local GPU training of M2, per
project policy).

Identical pipeline to scripts/kaggle_train_m1.py. The only intended difference
between M1 and M2 is the config the model factory reads
(src.config.EXPERIMENT_CONFIGS["M2"]["dropout"] = 0.3,
["recurrent_dropout"] = 0.2, ["spatial_dropout"] = 0.2) - this script does not
branch on experiment identity anywhere beyond that.

recurrent_dropout > 0 can disable the cuDNN fast path for LSTM, so this run is
expected to be materially slower than M1's - a recorded trade-off, not a reason
to change the config (project_plan.md §12).
"""
...
INPUT_DIR = Path("/kaggle/input/datasets/udbhav748/cfpb-m2-frozen-inputs")
...
def main() -> None:
    ...
    # 3. Frozen tokenizer (never refit) and sequences at M2's locked max_len=128 -
    # same tokenizer, same vocabulary, same max_len as M0/M1.
    ...
    # 4. Build M2 - random embeddings, no matrix passed. The only architecture
    # difference from M1 is config.dropout=0.3/recurrent_dropout=0.2/
    # spatial_dropout=0.2, which build_recurrent_model turns into a
    # SpatialDropout1D layer plus LSTM dropout kwargs - not a branch here.
    set_seed(SEED)
    ...
    # 6. Callbacks - for M2 this is exactly [ModelCheckpoint(val_macro_f1)],
    # enforced by the factory itself (no scheduler/early-stopping fields are set
    # in M2's config, so build_callbacks adds neither).
    ...
    # recurrent_dropout>0 can disable the cuDNN fast LSTM path, so this run is
    # expected to run slower than M1 per epoch - recorded, not treated as a bug.
    history = model.fit(
        X_train, y_train,
    ...
    run_record = {
        "experiment": EXPERIMENT, "model": "BiLSTM + Spatial Dropout",
